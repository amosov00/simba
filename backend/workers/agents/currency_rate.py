...
from schemas import Meta, MetaSlugs, MetaCurrencyRatePayload
from database.crud import MetaCRUD

__all__ = ["currency_rate_job"]

currency_rate_topic = app.topic(
    "currency_rate", internal=True, retention=timedelta(minutes=10), partitions=1
)
    ...
    async for _ in stream:
        ...
        if currency_rate:
            await MetaCRUD.update_by_slug(
                slug=MetaSlugs.CURRENCY_RATE,
                payload=MetaCurrencyRatePayload(BTCUSD=currency_rate).dict()
            )
        else:
