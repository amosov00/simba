...
export const actions = {
  ...
  transferSimbaToken({}, data) {
    const methodABI = this.$contract().SIMBA._jsonInterface.find((el) => el.name === 'transfer')
          ...
          params: [
            {
              ...
              data: web3.eth.abi.encodeFunctionCall(methodABI, methodInputs),
            },
          ],
        },
        (err, result) => {
  ...
  async fetchSimbaBalance({ rootGetters, commit }) {
    ...
    this.$contract()
      .SIMBA.methods.balanceOf(address)
      .call()
      .then((res) => {
